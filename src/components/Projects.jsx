...
const Projects = () => {
    ...
    const [projects] = useState([
        {
            ...
            description: "An e-commerce project built with React, offering product Browse and a modern responsive UI.",
    ...
    return (
        // تم تغيير الخلفية ولون النص العام ليتبعا ثيم DaisyUI
        <div id="projects" className="py-20 px-4 sm:px-10 bg-base-100 text-base-content">
            <div className="max-w-6xl mx-auto">
                <h2 className="text-4xl font-bold text-center mb-4">
                    My <span className="text-primary">Projects</span> {/* استخدام لون primary من الثيم */}
                </h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {projects.map((project) => (
                        <motion.div
                            key={project.name}
                            // تغيير خلفية الكارد وحدودها لتتبع الثيم
                            className="relative group rounded-lg overflow-hidden shadow-lg bg-base-200 border border-base-300 cursor-pointer"
                        ...
                        >
                            ...
                            <div className="absolute inset-0 bg-black/50 flex flex-col justify-center items-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                ...
                                <div className="flex items-center text-primary font-semibold"> {/* لون primary للنص المميز */}
                            ...
                            {!project.completed && (
                                // استخدام لون warning من ثيم DaisyUI
                                <div className="absolute top-3 right-3 bg-warning text-base-content text-xs font-bold px-3 py-1 rounded-full z-10">
                                    In Progress
                                </div>
                            )}
                ...
                {selectedProject && (
                    ...
                    >
                        <motion.div
                            ...
                            // خلفية المودال وحدوده تتبع الثيم
                            className="bg-base-200 border border-base-300 rounded-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto"
                            onClick={(e) => e.stopPropagation()} // لمنع إغلاق النافذة عند النقر داخلها
                        >
                            <div className="sticky top-0 bg-base-200/80 backdrop-blur-sm p-4 flex justify-between items-center z-10 border-b border-base-300">
                                <h2 className="text-2xl font-bold text-primary">{selectedProject.name}</h2> {/* عنوان المودال بلون primary */}
                                <button onClick={() => setSelectedProject(null)} className="text-base-content/70 hover:text-primary transition-colors">
                                    <X />
                                </button>
                            </div>
                            <div className="p-6">
                                <img src={selectedProject.image} alt={selectedProject.name} className="w-full rounded-lg mb-6 shadow-lg"/>
                                <p className="text-base-content mb-6 leading-relaxed">{selectedProject.description}</p>
                                <div className="flex flex-wrap gap-2 mb-6">
                                    {selectedProject.tags.map(tag => (
                                        // ألوان الـ tags تتبع الثيم
                                        <span key={tag} className="bg-base-300 text-primary text-sm font-medium px-3 py-1 rounded-full">{tag}</span>
                                    ))}
                                                                </div>
                                <div className="flex flex-wrap gap-4">
                                    <a href={selectedProject.liveUrl} target="_blank" rel="noopener noreferrer" 
                                       // زر Live Demo بلون primary
                                       className={`flex items-center gap-2 px-5 py-2 bg-primary text-black rounded-lg transition-all duration-300 hover:bg-primary/80 hover:scale-105 ${!selectedProject.completed || selectedProject.liveUrl === '#' ? "opacity-50 cursor-not-allowed pointer-events-none" : ""}`}>
                                        <ExternalLink size={18}/> Live Demo
                                    </a>
                                    <a href={selectedProject.sourceUrl} target="_blank" rel="noopener noreferrer" 
                                       // زر Source Code بلون base-300
                                       className={`flex items-center gap-2 px-5 py-2 bg-base-300 text-base-content rounded-lg transition-all duration-300 hover:bg-base-300/80 hover:scale-105 ${selectedProject.sourceUrl === '#' ? "opacity-50 cursor-not-allowed pointer-events-none" : ""}`}>
